...
from flask_app.models import user, book, review, friend, library
import re
...
class Wishlist:
    ...
    @classmethod
    def save(cls, data ):
        query = "INSERT INTO wishlists ( user_id , book_id, created_at, updated_at) VALUES ( %(user_id)s , %(book_id)s, NOW() , NOW() );"
    ...
    @classmethod
    def update(cls,data):
        query = "UPDATE wishlists SET user_id=%(user_id)s, book_id=%(book_id)s, updated_at = NOW() WHERE id = %(id)s;"
        return connectToMySQL('book_library').query_db(query,data)

    @classmethod
    def get_one(cls,data):
        query = "SELECT * FROM wishlists WHERE wishlists.id = %(id)s;"
    ...
    @classmethod
    def get_all_wishlist(cls, data):
        query = "SELECT * FROM books JOIN wishlists ON books.id  = wishlists.book_id JOIN users ON wishlists.user_id = users.id WHERE user_id = %(user_id)s ORDER BY author ASC;"
        results = connectToMySQL('book_library').query_db(query, data)

        if len(results):
            all_books_wishlist = []
            for books in results:
                all_books_wishlist.append( cls(books) )

            return all_books_wishlist


    @classmethod
    def delete_from_wishlist(cls,data):
        query = "DELETE FROM wishlists USING users LEFT JOIN wishlists on users.id = wishlists.user_id LEFT JOIN books ON wishlists.book_id = books.id WHERE user_id = %(user_id)s AND book_id = %(id)s;"
        return connectToMySQL('book_library').query_db(query,data)


    @classmethod
    def delete(cls,data):
        query = "DELETE FROM wishlists WHERE id = %(id)s;"
        return connectToMySQL('book_library').query_db(query,data)
